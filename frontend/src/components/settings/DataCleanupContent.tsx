...
interface Props {
  /** If true, syncs the active sub-tab to `?cleanup_tab=...` so it
   *  survives reloads even when nested under another page. */
  syncToUrl?: boolean;
}

export function DataCleanupContent({ syncToUrl = false }: Props) {
  const [searchParams, setSearchParams] = useSearchParams();
  const urlKey = syncToUrl ? 'cleanup_tab' : 'tab';
  const initialTab = (searchParams.get(urlKey) as CleanupTab) || 'needs_classification';
  const [tab, setTab] = useState<CleanupTab>(initialTab);

  useEffect(() => {
    if (!syncToUrl) return;
    const next = new URLSearchParams(searchParams);
    if (tab === 'needs_classification') next.delete(urlKey);
    else next.set(urlKey, tab);
    ...
    <Tabs value={tab} onValueChange={(v) => setTab(v as CleanupTab)}>
        ...
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-border/60 bg-muted/30">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              ...
              or <strong>Remove</strong> if they don't belong.
            </p>
          </div>
        ...
        <Card className="p-6 space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Two existing scans for duplicate people — one keyed on overlapping resume identities
      ...
      <TabsContent value="enrichment_ambiguous" className="mt-4">
        <Card className="p-6">
          ...
          {ambiguousCount === 0 ? (
            <p className="text-xs text-muted-foreground italic">No ambiguous matches right now.</p>
          ) : (
        ...
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-border/60 bg-muted/30">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              ...
              waterfall is wired, this page will offer a one-click enrich-all button. For now, click through
              and edit manually.
            </p>
          </div>
          <MissingChannelList />
...
function MissingChannelList() {
  ...
  if (isLoading) return <div className="p-6 text-xs text-muted-foreground">Loading…</div>;
  if (rows.length === 0) {
    return (
