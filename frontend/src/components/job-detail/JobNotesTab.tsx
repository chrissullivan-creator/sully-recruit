...
export function JobNotesTab({ jobId }: JobNotesTabProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setDraft('');
      queryClient.invalidateQueries({ queryKey: ['job_notes', jobId] });
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-card-border bg-white p-4">
        <textarea
          ...
          placeholder="Add a note about this job — call summary, client preferences, anything the team should see…"
          rows={3}
