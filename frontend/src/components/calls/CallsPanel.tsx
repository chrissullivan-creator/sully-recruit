...
// ---- Calls Panel ----
// The full Calls experience (call-log list, search, Log Call dialog,
// CallDetailModal). Rendered both as the standalone /calls page (via the
// Calls.tsx wrapper) and embedded inside the Communication Hub. When
// `embedded` is true the outer PageHeader is suppressed (the Hub supplies
// its own) and spacing is adapted to sit inside the Hub's content area.
export function CallsPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <>
      ...
      <div className={cn(
        embedded
          ? 'h-full overflow-y-auto bg-page-bg p-6'
          : 'bg-page-bg min-h-[calc(100vh-4rem)] p-6 lg:p-8',
      )}>
        {/* Embedded: the Hub renders its own header, so surface the Log Call
            action + title inline at the top of the panel. */}
        {embedded && (
