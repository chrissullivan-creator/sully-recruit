...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { data: link, error: linkErr } = await supabase
      ...
      .select(
        "id, owner_user_id, integration_account_id, slug, title, duration_min, meeting_type, location, timezone, working_hours, buffer_min, min_notice_hours, max_days_out, max_per_day, active",
      )
