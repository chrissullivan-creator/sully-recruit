-- Add avatar + company-logo fields to the command center preview lists so the
-- dashboard can show profile pictures and company logos (falls back to initials
-- client-side via PersonAvatar / CompanyLogo).
...
rtm_list as (select coalesce(jsonb_agg(x),'[]'::jsonb) j from (
    select p.id, p.full_name name, p.current_title title, p.current_company company,
      p.last_sequence_sentiment sentiment, coalesce(p.profile_picture_url, p.avatar_url) avatar,
      c.domain company_domain, c.logo_url company_logo
...
bm_list as (select coalesce(jsonb_agg(x),'[]'::jsonb) j from (
    select p.id, p.full_name name, p.current_title title, p.current_company company,
      p.current_total_comp cur, p.target_total_comp tgt, coalesce(p.profile_picture_url, p.avatar_url) avatar,
      c.domain company_domain, c.logo_url company_logo
    from people p left join companies c on c.id = p.company_id
    where p.type='candidate' and p.deleted_at is null and p.current_total_comp is not null and p.target_total_comp is not null and p.target_total_comp > p.current_total_comp*1.1
...
risk_list as (select coalesce(jsonb_agg(x),'[]'::jsonb) j from (
    ...
    where j.status in ('hot','offer_made') and j.deleted_at is null
      and not exists (select 1 from candidate_jobs cj where cj.job_id=j.id and cj.deleted_at is null and cj.stage_updated_at>now()-interval '14 days')
      and not exists (select 1 from send_outs so where so.job_id=j.id and so.deleted_at is null and so.updated_at>now()-interval '14 days')
      and (j.last_sourced_at is null or j.last_sourced_at < now()-interval '14 days')
