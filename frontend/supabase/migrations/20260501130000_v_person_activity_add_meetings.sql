-- Pass 7b: extend v_person_activity to include calendar/meeting events from tasks.
-- Tasks with start_time (or calendar_event_id) are meetings. Connect to people via:
--   * meeting_attendees (entity_type, entity_id) when the task is a meeting with attendees
--   * tasks.related_to_type/related_to_id when no meeting_attendees rows exist
-- Also enriched the 'interview' activity row with calendar fields (event id, url, attendees).
...
WITH (security_invoker = true) AS
  ...
  UNION ALL
  -- Calendar/Outlook meetings: tasks linked to a person via meeting_attendees or related_to_id
  SELECT 'meeting',
         COALESCE(ma.entity_id, t.related_to_id) AS person_id,
         COALESCE(t.start_time, t.created_at)    AS happened_at,
         COALESCE(t.title, 'Meeting')            AS summary,
         jsonb_build_object(
           'start_time', t.start_time, 'end_time', t.end_time,
           'location', t.location, 'meeting_url', t.meeting_url,
           'meeting_provider', t.meeting_provider,
           'calendar_event_id', t.calendar_event_id, 'task_type', t.task_type,
           'description', t.description, 'status', t.status
         ),
         t.assigned_to AS actor_user_id, 'tasks', t.id
  FROM tasks t
  LEFT JOIN meeting_attendees ma ON ma.task_id = t.id AND ma.entity_type IN ('candidate','contact')
  WHERE (t.start_time IS NOT NULL OR t.calendar_event_id IS NOT NULL)
    AND COALESCE(ma.entity_id, t.related_to_id) IS NOT NULL
  ...
  SELECT 'interview', candidate_id, COALESCE(scheduled_at, created_at),
         ...
         jsonb_build_object('job_id', job_id, 'stage', stage, 'round', round, 'outcome', outcome,
           'interviewer_name', interviewer_name, 'meeting_link', meeting_link, 'calendar_event_id', calendar_event_id,
           'calendar_event_url', calendar_event_url, 'calendar_attendees', calendar_attendees),
...
COMMENT ON VIEW v_person_activity IS
  'Unified per-person activity timeline. Includes messages, calls, ai_notes, status changes, stage moves, notes, calendar/Outlook meetings (from tasks), pitches, sendouts, submissions, interviews, placements, rejections, merges. Filter by person_id for record-of-truth feed.';
