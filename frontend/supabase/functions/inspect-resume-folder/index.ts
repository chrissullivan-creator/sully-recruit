...
Deno.serve(async (req) => {
  ...
  // Grab 5 sample messages from the folder with full details
  const r = await fetch(
    `https://graph.microsoft.com/v1.0/me/mailFolders/${encodeURIComponent(FOLDER_ID)}/messages?$top=5&$select=id,subject,from,receivedDateTime,hasAttachments,body,bodyPreview`,
    { headers: { Authorization: `Bearer ${token}` } }
  );
  const data = await r.json();

  // Also fetch attachments for first message
  const samples = [];
  ...
  return new Response(JSON.stringify({ total_in_folder: data["@odata.count"], samples }, null, 2), {
